...
class CacheFtp extends CacheModel
{
    ...
    public function canUse(): bool
    {
        ...
        if (!$rslt) {
            CoreLogger::addDebug('FTP function not found');
        }
        return $rslt;
    }
    ...
    public function netConnect(): void
    {
        if (!$this->netConnected()) {
            ...
            $this->setConnectionObject(ftp_connect($this->getTransactionHost(),
                                                   $this->getServerPort(),
                                                   $this->timeOut));

            if ($this->getConnectionObject() === false) {
                CoreLogger::addDebug('Could not connect to host ' . $this->getTransactionHost() . ' on port ' . $this->getServerPort());
    ...
    {
        if ($this->netConnected()) {
            if (ftp_close($this->getConnectionObject())) {
    ...
    public function &netSelect(): bool
    {
        ...
        if (ftp_login($this->getConnectionObject(),
            ...
            CoreLogger::addDebug('Unable to login.');
        }
        return $rslt;
    }
    ...
    public function &getNameList(string $path): array
    {
        ...
        if ($this->netConnected()) {
            ...
            if ($this->setPassiveMode()) {
                // Recherche la liste
                $dirList = ftp_nlist($this->getConnectionObject(),
                                     $this->getRootPath($path));

                // Si aucune erreur, on nettoie
                if (!is_bool($dirList)) {
                    $dirList = preg_replace('#^' . preg_quote($this->getRootPath($path),
                                                                                 '#') . '[/\\\\]?#',
                                                                                 '',
                                                                                 $dirList);
                }
    ...
    public function &getCacheMTime(string $path): int
    {
        ...
        if ($this->netConnected()) {
            $mTime = ftp_mdtm($this->getConnectionObject(),
                              $this->getRootPath($path));

            if ($mTime === -1) { // Une erreur est survenue
                CoreLogger::addDebug('Bad response for ftp_mdtm command. Path : ' . $path
    ...
    {
        $rslt = ftp_set_option($this->getConnectionObject(),
                               FTP_TIMEOUT_SEC,
                               $this->timeout);
        return $rslt;
    }
    ...
    private function chmod(string $path,
                           int $mode): void
    {
        if (ftp_site($this->getConnectionObject(),
                     'CHMOD ' . $mode . ' ' . $this->getRootPath($path))) {
            CoreLogger::addDebug('Bad response for ftp_site CHMOD command. Path : ' . $path);
        }
    }
...
//$path : local => chemin valide local jusqu'au fichier, remote => chemin valide FTP (avec le root donc) jusqu'au fichier
        if ($this->netConnected()) {
            ...
            if ($this->setPassiveMode()) {
                ...
                $buffer = fopen($this->getRootPath($path),
                                                   'a'); // TODO il faut mettre un path local ici !
                fwrite($buffer,
                       $content);
                rewind($buffer);

                // Ecriture du fichier
                if (!ftp_fget($this->getConnectionObject(),
                              $buffer,
                              $this->getRootPath($path),
                                                 FTP_ASCII)) {// TODO il faut mettre une path remote ici !
                    CoreLogger::addDebug('Bad response for ftp_fget command. Path : ' . $path);
                }
    ...
    {
        if (!ftp_mkdir($this->getConnectionObject(),
                       $path)) {
            CoreLogger::addDebug('Bad response for ftp_mkdir command. Path : ' . $path);
        }
                                ...
                                int $timeLimit): void
    {
        ...
        if ($deleteFile && $this->netConnected()) {
            ...
            if (!ftp_delete($this->getConnectionObject(),
                            $this->getRootPath($path))) {
                CoreLogger::addDebug('Bad response for ftp_delete command. Path : ' . $path);
            }
        }
    }
                                     ...
                                     int $timeLimit): void
    {
        ...
        if ($timeLimit === 0 && $this->netConnected()) {
            if (!ftp_rmdir($this->getConnectionObject(),
                           $this->getRootPath($path))) {
                CoreLogger::addDebug('Bad response for ftp_rmdir command. Path : ' . $path);
            }
        }
    }
    ...
    private function &setPassiveMode(): bool
    {
        ...
        if ($this->netConnected()) {
            if (ftp_pasv($this->getConnectionObject(),
                         true)) {
                $rslt = true;
            }
        }
