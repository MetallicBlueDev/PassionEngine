...
if (isset($_GET['mode']) && $_GET['mode'] == "code" && !empty($_GET['code'])) { // Methode captcha code
...
else if (isset($_GET['mode']) && $_GET['mode'] == "text" && !empty($_GET['text'])) { // Methode text & mail
    // Décryptage
    $text = $_GET['text'];
    $text = urldecode($text);
    ...
    if (strlen($text) < 5)
        ...
        $more_pixel = -((strlen($text) - 60) / 1.8);
    else if (strlen($text) > 60)
        $more_pixel = -((strlen($text) - 60));
    else if (strlen($text) > 50)
        $more_pixel = -((strlen($text) - 50));
    else
        $more_pixel = 0;
